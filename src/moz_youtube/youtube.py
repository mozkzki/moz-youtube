...
def create_youtube(dotenv_path: str = None) -> Resource:
    ...
    if credentials is None or credentials.invalid:
        ...
        Path("moz_youtube_token_save.json").write_text(credentials.to_json())
...
def _create_client_secret() -> str:
    secret_path = "/tmp/moz_youtube_client_secret.json"
    with open(secret_path, "w") as st:
        secret = os.environ.get("moz_youtube_client_secret_contents", "dummy")
        st.write(secret)
        st.close()
    return secret_path
...
def _create_token() -> str:
    oauth_path = "/tmp/moz_youtube_token.json"
    with open(oauth_path, "w") as oa:
        contents = os.environ.get("moz_youtube_token_contents", "dummy")
        oa.write(contents)
        oa.close()
    return oauth_path
